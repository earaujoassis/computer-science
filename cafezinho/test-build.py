...
def main(argv):
    if not os.path.isfile('./cafezinho'):
        print("{0}{1}{2}".format(
            TColors.FAIL, "Error: you must build the cafezinho compiler (make build)", TColors.ENDC))
        exit(1)
    if not os.path.isfile('tests/descriptor.json'):
        print("{0}{1}{2}".format(
            TColors.FAIL, "Error: tests/descriptor.json file is missing", TColors.ENDC))
        exit(1)
    ...
    for test in tests:
        ...
        return_code = subprocess.call(
            arguments, stdout=FNULL, stderr=subprocess.STDOUT, shell=False)
        if (return_code == test['status']):
            print("{0}{1}{2}{3}\t{4}{5}{6}".format(
                "[", TColors.OKGREEN, "OK", TColors.ENDC, "]", " for ", test['file']))
        else:
            print("{0}{1}{2}{3}\t{4}{5}{6}{7}{8}".format(
                "[", TColors.FAIL, "ERROR", TColors.ENDC, "]", " for ", test['file'], ": ", test['unexpected']))
    FNULL.close()
