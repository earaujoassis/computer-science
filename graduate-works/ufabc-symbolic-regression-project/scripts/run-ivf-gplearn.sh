...
mkdir -p ./results
mkdir -p ./results/ivf-gplearn

for srsd_category in easy medium hard; do
    RESULT_DIR=./results/ivf-gplearn/srsd-feynman_${srsd_category}
    mkdir -p ${RESULT_DIR}
    ...
    for filepath in ./datasets/srsd-feynman_${srsd_category}/train/*; do
        ...
        python ./runners/solution_ivf_gplearn.py --config ./configs/config.ivf-gplearn.yaml --train ${TRAIN_FILE} --val ${VAL_FILE} --test ${TEST_FILE} --out ${RESULT_DIR}/${FILE_NAME}.csv
    done
done
