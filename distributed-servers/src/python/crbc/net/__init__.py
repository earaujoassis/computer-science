...
class CRBCRegistry(UDPTwoWay):
  ...
  def __remove_entry(self, entry_name):
    if entry_name in self.__mapping:
      ...
      print("Entry removed \"{0}\" for {1}".format(entry_name, client_address))
  ...
  def finish_request(self, request, client_address):
    ...
    if datagram.method == CRBCEstablishment.Register:
      if datagram.name in self.__mapping.keys():
        ...
        if not ttl == 0:
          Timer(ttl, self.__remove_entry, [datagram.name], {}).start()
      ...
      if datagram.name in self.__mapping.keys():
        ...
        received = super(CRBCRegistry, self).send_message(datagram.writable(), *destination)
...
class CRBCDummyNode(UDPTwoWay):
  ...
  def registration_process(self, host, port, edict={}):
    ...
    received = super(CRBCDummyNode, self).send_message(establishment.writable(), "localhost", 9999)
    response = CRBCEstablishment(complete=received)
    if response.method != CRBCEstablishment.Accept:
      raise NonEstablished("Couldn't establish registration for {}".format(self.__name))
    else:
  ...
  def send_message(self, name, message):
    ...
    received = super(CRBCDummyNode, self).send_message(communication.writable(), *self.__main_register)
  ...
  def reset_name(self, name):
    if not self.__main_register:
      ...
      raise NameChangingPostEstablishment("Can't change it post-establishment")
